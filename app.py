...
def get_instruments_df():
    ...
    return pd.concat(all_dfs, ignore_index=True)[columns]
...
if symbol:
    with st.spinner("Buscando (primera vez puede tardar ~20 seg)..."):
        df = get_instruments_df()  # Usa cache después del primer acceso
